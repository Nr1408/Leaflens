...
export default function HomeScreen({ navigation }: Props) {
  ...
  const handleResultNavigate = async (imageUri: string, json: any) => {
    ...
    // Apply client-side rejection threshold (mirrors server). Allows tuning without redeploying server.
    const threshold = typeof json?.threshold === 'number' ? json.threshold : 0.40;
    const top1 = serverTop1.probability < threshold
      ? { label: 'Unknown', probability: serverTop1.probability }
      : serverTop1;
