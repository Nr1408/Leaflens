...
export default function ResultScreen({ route, navigation }: Props) {
  ...
  const isNoDisease = label === 'No disease found';
  ...
  return (
    <Screen>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing(2) }}>
        ...
        <Text style={{ fontSize: 24, fontWeight: '800', color: colors.text }}>{isNoDisease ? label : (info?.name ?? label)}</Text>
        {!isNoDisease && (
        ...
        {/* Optional: show top alternatives for transparency (only when a disease is detected) */}
        {!isNoDisease && topK && topK.length > 1 && (
          <Card>
