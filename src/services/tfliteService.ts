...
const BUNDLED_TFLITE = require('../../assets/models/model_fp16.tflite');
...
export async function diagnoseImage(imageUri: string, _opts: Record<string, unknown> = {}): Promise<LocalDiag> {
	...
	if (__DEV__) {
		...
		// eslint-disable-next-line no-console
		console.log('[LeafLens] Bundled TFLite asset id:', BUNDLED_TFLITE, 'Platform:', Platform.OS, 'Image URI:', imageUri);
	}
